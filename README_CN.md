...
### 5. 📊 图表支持 (Diagrams)
- **Mermaid**：流程图、时序图、甘特图等，在预览中完美渲染。
- **Draw.io**：嵌入的 Draw.io XML 图表可直接渲染。
- **Excalidraw**：支持 Excalidraw 手绘风格图表的 JSON 格式。

### 6. 🔗 导航与链接
- **目录导航**：侧边栏显示文档标题结构，点击可跳转到任意章节。
- **内部锚点链接**：点击 `[链接](#标题)` 可平滑滚动到目标标题。
- **外部链接**：点击可在默认浏览器中打开。

### 7. 🎨 主题与显示选项
- **多种主题**：支持深蓝色（默认）、黑色（VS Code 风格）、浅色、Solarized Light、Solarized Dark、Monokai 六种主题。
- **全宽模式**：切换 A4 固定宽度与全窗口宽度显示。
- **全屏模式**：进入全屏沉浸阅读（按 ESC 键退出或点击悬浮退出按钮）。
- **另存为**：将当前文档保存为新文件，自动建议文件名（原文件名 + "_update"）。
- **导出 HTML**：将当前文档导出为带样式的 HTML 文件，支持深色/浅色主题切换（文件存在时提示）。
- **会话恢复**：程序重启时自动恢复上次关闭前打开的所有文档（仅保存文件路径，减小配置文件大小）。
...
*   **渲染**: react-markdown + rehype-highlight + mermaid (图表支持)。
